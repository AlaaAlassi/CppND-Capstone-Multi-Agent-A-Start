...
int main(int argc, char **argv)
{
    ...
    auto rob4 = std::make_shared<Robot>(4, warehouse._map.getCell(1,33), warehouse._map.getCellSize() * 0.5);
    std::deque<std::shared_ptr<Robot>> busyRobots;
    busyRobots.push_back(rob1);
    busyRobots.push_back(rob2);
    busyRobots.push_back(rob3);
    busyRobots.push_back(rob4);
    viewer.setRobots(busyRobots);
    viewer.loadBackgroundImg();

    // run viewer thread
    std::thread simulationThread(&Graphics::run, &viewer);
    pair<shared_ptr<CellData>, shared_ptr<CellData>> task(warehouse._map.getCell(0,33),warehouse._map.getCell(0,20));
    int t0 = 0;
    ...
    t0 = t0+1;
    pair<shared_ptr<CellData>, shared_ptr<CellData>> task4(warehouse._map.getCell(2,0),warehouse._map.getCell(0,20));
